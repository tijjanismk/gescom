...
pub fn migrer(conn: &Connection) -> Result<()> {
    ...
    // suivent. Le meme DDL que pose `amorcage::cloisonnement`, cote
    // Base — les deux doivent rester identiques.
    // Pas de `REFERENCES dossier(id)` : aucune autre table cloisonnee
    // n'en porte, voir amorcage.rs::cloisonnement.
    conn.execute(
    ...
    conn.execute(
        "INSERT OR IGNORE INTO dossier (id, code, societe, clos, cree_le, modifie_le)
         VALUES (?1, 'PRINCIPAL', 'Ma boutique', 0, ?2, ?2)",
        rusqlite::params![defaut, maintenant],
    )
    .ok();
